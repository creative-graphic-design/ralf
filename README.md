...
We recommend using uv (all settings and dependencies in [pyproject.toml](pyproject.toml)).

### 2. How to install

#### Local environment
1. Install uv (see [official docs](https://docs.astral.sh/uv/)).
...
```bash
uv run python -m ralf.hfds_builder.inpainting --dataset_root <DATASET_ROOT>
```

### Saliency detection

```bash
uv run python -m ralf.hfds_builder.saliency_detection --input_dir <INPUT_DIR> --output_dir <OUTPUT_DIR> (--algorithm (isnet|basnet))
```

### Aggregate data and dump to HFDS
```bash
uv run python -m ralf.hfds_builder.dump_dataset --dataset_root <DATASET_ROOT> --output_dir <OUTPUT_DIR>
```
...
Please customize [src/ralf/train/inference_single_data.py](https://github.com/CyberAgentAILab/RALF/blob/main/src/ralf/train/inference_single_data.py) to load your data.
