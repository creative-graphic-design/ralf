...
from ralf.train.helpers.rich_utils import CONSOLE
...
def main(args):
    ...
    for split in ["train", "val", "test"]:
        indexes_path = os.path.join(
            CACHE_DIR,
            f"{args.dataset}_{split}_{args.retrieval_backbone}_wo_head_table_between_dataset_indexes_top_k{args.rerank_pool_size}.pt",
        )
        ...
        reranked_indexes_path = os.path.join(
            CACHE_DIR,
            f"{args.dataset}_{split}_{args.retrieval_backbone}_{params_str}_wo_head_table_between_dataset_indexes_top_k{args.top_k}.pt",
        )
...
# CUDA_VISIBLE_DEVICES=5 OMP_NUM_THREADS=4 uv run python -m ralf.preprocess.rerank_indexes --rerank_type mmr --rerank_mmr_lam 0.99 --dataset_path /home/jupyter/datasets/image_conditioned_layout_generation/preprocessed_parquets --rerank_pool_size 64 --dataset cgl --fid_weight_dir tmp/fidnet/cgl --dataset cgl
