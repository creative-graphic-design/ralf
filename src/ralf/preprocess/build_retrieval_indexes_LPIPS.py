...
def preprocess_retriever(
    ...
    DATABASE = {}
    for j in tqdm(range(len(datasets["train"])), desc="Caching database"):
    ...
    for split in datasets.keys():
        ...
        cache_path = os.path.join(
            CACHE_DIR,
            f"{dataset_name}_{split}_lpips_wo_head_table_between_dataset_indexes_top_k{top_k}.pt",
        )
...
# CUDA_VISIBLE_DEVICES=1 OMP_NUM_THREADS=4 uv run python -m ralf.preprocess.build_retrieval_indexes_LPIPS --dataset_name pku --dataset_path /datasets/PosterLayout

# CUDA_VISIBLE_DEVICES=2 OMP_NUM_THREADS=4 uv run python -m ralf.preprocess.build_retrieval_indexes_LPIPS --dataset_name cgl --dataset_path /datasets/PosterLayout
