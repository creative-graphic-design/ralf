...
class CGLGenerator(BaseGANGenerator):
    def __init__(
        ...
        expected_in_channels = 2 * (features["label"].feature.num_classes + 1)
        if in_channels != expected_in_channels:
            logger.warning(
                "Adjusting CGLGenerator in_channels from %s to %s to match features.",
                in_channels,
...
class CGLDiscriminator(BaseModel):
    ...
    def __init__(
        ...
        if in_channels != expected_in_channels:
            logger.warning(
                "Adjusting CGLDiscriminator in_channels from %s to %s to match features.",
                in_channels,
                expected_in_channels,
            )
            in_channels = expected_in_channels
