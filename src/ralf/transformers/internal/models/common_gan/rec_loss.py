#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# From https://github.com/PKU-ICST-MIPL/PosterLayout-CVPR2023/blob/main/RecLoss.py
...
from .design_seq import box_cxcywh_to_xyxy, box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs: Dict[str, Tensor], targets: List[Dict[str, Tensor]]):
        ...
        assert out_bbox.size(-1) == 4

        tgt_ids = torch.cat([v["labels"] for v in targets])
        tgt_bbox = torch.cat([v["boxes"][..., :4] for v in targets])
...
def linear_sum_assignment_with_inf(cost_matrix, INFTY_COST=1e5):
...
class SetCriterion(nn.Module):
    ...
    1) we compute hungarian assignment between ground truth boxes and the outputs
       of the model
    2) we supervise each pair of matched ground-truth / prediction (supervise
       class and box)
    """
    ...
    def __init__(self, num_classes, matcher, weight_dict, coef, losses):
        ...
        Parameters:
            ...
            eos_coef: relative classification weight applied to the no-object
                category
            losses: list of all the losses to be applied. See get_loss for list
                of available losses.
        """
        super().__init__()
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        # loss_ce = F.cross_entropy(src_logits.transpose(1, 2).float(), target_classes.long(), self.empty_weight)
        loss_ce = F.cross_entropy(
            src_logits.transpose(1, 2).float(),
            target_classes.long(),
            self.empty_weight.float(),
        )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_boxes = torch.cat(
            [t["boxes"][i][..., :4] for t, (_, i) in zip(targets, indices)], dim=0
        )
