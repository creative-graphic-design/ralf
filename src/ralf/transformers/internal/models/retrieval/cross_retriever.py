...
class CrossRetriever(BaseModel):
    ...
    def load_faiss_index(self, dataset_name, retrieval_backbone, db_dataset):
        faiss_index_file_name = os.path.join(
            CACHE_DIR, f"{dataset_name}_{retrieval_backbone}_wo_head_index.faiss"
        )
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i in range(B):
            ...
            # from ralf.transformers.internal.helpers.visualizer import render
    ...
    def preprocess_retrieval_cache(
        ...
        cache_path = os.path.join(
            CACHE_DIR,
            f"{self.dataset_name}_{split}_{self.retrieval_backbone}_cross_dataset_indexes_top_k{top_k}.pt",
        )
        ...
        for dataset in ["pku", "cgl"]:
            cache_table_paired_path = os.path.join(
                CACHE_DIR,
                f"{dataset}_{self.retrieval_backbone}_cache_table_paired.pt",
            )
