...
from ralf.transformers.internal.global_variables import CACHE_DIR
from ralf.transformers.internal.helpers.rich_utils import get_progress
from ralf.transformers.internal.helpers.util import pad
...
class Retriever(BaseModel):
    ...
    Usage:
        bash bin/inference.sh \
            job_dir=ralf/train/dummy_config/non_learnable \
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "merge" in retrieval_backbone or "concat" in retrieval_backbone:
            ...
            # Faiss
            self.faiss_index_file_name = os.path.join(
                self.cache_dir,
                f"{dataset_name}_{retrieval_backbone}_wo_head_index.faiss",
            )
            self.backbone = FeatureExtracterBackbone(
                db_dataset=db_dataset, retrieval_backbone=self.retrieval_backbone
            )
            cache_exists = os.path.exists(self.faiss_index_file_name)
            cache_valid = (
                cache_exists and os.path.getsize(self.faiss_index_file_name) > 0
            )
            if cache_valid:
                try:
                    ...
                    logger.warning(
                        "Faiss cache load failed; rebuilding index from dataset."
                    )
                    cache_valid = False
            if not cache_valid:
                if not cache_exists:
    ...
    def preprocess_retrieval_cache(
        ...
        cache_path = os.path.join(
            self.cache_dir,
            f"{self.dataset_name}_{split}_{self.retrieval_backbone}_wo_head_table_between_dataset_indexes_top_k{top_k}.pt",
        )
        ...
        cache_table_paired_path = os.path.join(
            self.cache_dir,
            f"{self.dataset_name}_{self.retrieval_backbone}_cache_table_paired.pt",
        )
    ...
    def preprocess_to_merge_retrieval_cache(
        ...
        cache_dataid_dbidx_path = os.path.join(
            self.cache_dir,
            f"{self.dataset_name}_train_saliency_cache_table_paired.pt",
        )
        ...
        cache_db_path = os.path.join(
            self.cache_dir,
            f"{dataset_name}_{split}_{self.retrieval_backbone}_{where_norm}__topk{top_k}.pt",
        )
