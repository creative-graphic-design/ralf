...
from ralf.transformers.internal.global_variables import PRECOMPUTED_WEIGHT_DIR
...
class _MappedUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "image2layout.train.helpers.relationships":
            module = "ralf.transformers.internal.helpers.relationships"
...
class RelationshipPreprocessor(BasePreprocessor):
    ...
    def __init__(self, RELATION_SIZE: int = 10, **kwargs: Any) -> None:
        ...
        relationship_table_path = os.path.join(
            os.environ.get("RALF_CACHE_DIR", "./cache"),
            "pku_cgl_relationships_dic_using_canvas_sort_label_lexico.pt",
        )
    ...
    def __call__(
        ...
        if not self.global_task_embedding:
            ...
            assert self.name_to_id("label") not in list(
