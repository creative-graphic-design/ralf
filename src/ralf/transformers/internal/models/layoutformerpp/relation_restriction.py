...
from ralf.transformers.internal.helpers.relationships import (
    REL_SIZE_ALPHA,
    RELATIVE_RELATION,
    RelElement,
    RelLoc,
...
from ralf.transformers.internal.models.layoutformerpp.task_preprocessor import (
...
class TransformerSortByDictLabelConstraint:
    def __init__(
        ...
        self.index2label = self.tokenizer._label_feature.names
        self.special_token_ids = set(self.tokenizer._special_token_name_to_id.values())
        self.num_token_ids = set(
            range(
                self.tokenizer.N_label,
                self.tokenizer.N_label + self.tokenizer.N_bbox,
            )
        )
        self.label_suffix = {self.tokenizer.name_to_id("eos")}
...
def detect_label_idx(types, label, index, map_relele_to_idx, preprocessor):
    ...
    if idx >= indexes.size(0):
        idx = indexes.size(0) - 1
        ...
        for kdx, rel in enumerate(_relations):
            ...
            if is_canvas:
                ...
                if label_j_pos == label_i_pos:
                    # Skip ambiguous relations that resolve to the same element.
                    continue
                ...
                assert label_i_pos > label_j_pos, (
