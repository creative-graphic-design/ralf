...
from ralf.transformers.internal.helpers.util import convert_xywh_to_ltrb
...
def _relation_loc(
    ...
    cond &= (graph.edge_attributes & 1 << rel_value).ne(0)
    left, top, right, bottom = convert_xywh_to_ltrb(bbox_flatten.permute(2, 0, 1))

    z = graph.edge_indexes
    li, lj = safe_gather(left, z[..., 0]), safe_gather(left, z[..., 1])
    ti, tj = safe_gather(top, z[..., 0]), safe_gather(top, z[..., 1])
    ri, rj = safe_gather(right, z[..., 0]), safe_gather(right, z[..., 1])
    bi, bj = safe_gather(bottom, z[..., 0]), safe_gather(bottom, z[..., 1])
