...
from ralf.transformers.internal.global_variables import GEO_KEYS, PRECOMPUTED_WEIGHT_DIR
...
def init_layout_tokenizer(
    ...
    if tokenizer_cfg.geo_quantization == "kmeans":
        if "pku" in dataset_cfg.name:
            _name = f"{dataset_cfg.name}{max_seq_length}_kmeans_train_clusters.pkl"
        else:
            _name = f"{dataset_cfg.name}_kmeans_train_clusters.pkl"
        local_path = os.path.join(PRECOMPUTED_WEIGHT_DIR, "clustering", "cache", _name)
        if fsspec.filesystem("file").exists(local_path):
...
class LayoutTokenizer:
    ...
    def convert_tokens_to_ids(self, tokens: list[str]) -> list[int]:
        label_to_id = {name: idx for idx, name in enumerate(self._label_feature.names)}
        ids: list[int] = []
        for token in tokens:
            if token in label_to_id:
                ids.append(label_to_id[token])
                continue
            if token in self._special_token_name_to_id:
                ids.append(self._special_token_name_to_id[token])
                continue
            try:
                value = int(token)
            except ValueError as exc:
                raise KeyError(f"Unknown token: {token}") from exc
            if self._is_loc_vocab_shared:
                ids.append(self.N_label + value)
            else:
                geo_vars = [key for key in self.var_order if key != "label"]
                first_geo = geo_vars[0]
                offset = GEO_KEYS.index(first_geo) * self.N_bbox_per_var
                ids.append(self.N_label + offset + value)
        return ids
