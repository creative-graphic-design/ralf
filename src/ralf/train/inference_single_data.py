...
def main(test_cfg: DictConfig) -> None:
    ...
    if "pku" in test_cfg.dataset_path:
        cache_table_path = os.path.join(
            ...
            "dataset",
            "pku_DATAID_TO_IDX.pt",
        )
    elif "cgl" in test_cfg.dataset_path:
        cache_table_path = os.path.join(
            os.environ.get("RALF_CACHE_DIR", "./cache"),
            "dataset",
            "cgl_DATAID_TO_IDX.pt",
        )
    ...
    for ckpt_path in ckpt_paths:
        ...
        aux_task = getattr(model, "auxilary_task", None)
        if aux_task != "refinement":
