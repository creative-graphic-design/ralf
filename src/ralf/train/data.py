...
from ralf.train.global_variables import DUMMY_LAYOUT, RETRIEVED_KEYS
from ralf.train.helpers.hfds_instance_wise_transforms import (
    hfds_instance_wise_trasnform_factory,
)
from ralf.train.helpers.metric import compute_validity
...
def collate_fn(
    ...
    """
    assert validity_check is None or validity_check == compute_validity, (
...
def get_dataset(
    ...
    if dataset_cfg.path:
        ...
        cache_dir = os.environ.get("RALF_CACHE_DIR", "./cache")
        dataset_root = os.environ.get(
            "RALF_DATASET_DIR", os.path.join(cache_dir, "dataset")
        )
        data_dir: str = dataset_cfg.data_dir or dataset_root
