...
class _TimmInceptionV3Wrapper(nn.Module):
    ...
    def __init__(self) -> None:
        super().__init__()
        logger.info("Loading timm model from inception_v3")
        device_name = "cuda" if torch.cuda.is_available() else "cpu"
...
def compute_rshm(
    ...
    device = next(vgg16.model.parameters()).device  # type: ignore[attr-defined]
    with torch.no_grad():
        logits = vgg16(images.to(device)).detach().cpu()  # type: ignore
        logits_masked = vgg16(images_masked.to(device)).detach().cpu()  # type: ignore
