...
from ralf.train.global_variables import CACHE_DIR, PRECOMPUTED_WEIGHT_DIR
...
class RetrievalDatasetWrapper(torch.utils.data.Dataset):
    def __init__(
        ...
        cache_path = os.path.join(
            CACHE_DIR,
            f"{self.dataset_name}_{split}_{retrieval_backbone}_wo_head_table_between_dataset_indexes_top_k{num_cache_indexes_per_sample}.pt",
        )
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for data_id in data_ids:
            ...
            assert len(retrieved_indexes) == self.top_k, (
