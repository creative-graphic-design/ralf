...
def load_k_scores(ROOT):
    ...
    dirs = glob(os.path.join(ROOT, "no_anno_data_*"))
    dirs = [d for d in dirs if "debug" not in d]
    SCORES = {}

    for d in dirs:
        task_name = d.split("/")[-1].split("_")[3]
        if "dynamictopk" in d:
        ...
        if not os.path.exists(score_path) or task_name not in unique_tasks:
        ...
        metric_name = _score[1:11]
        assert metric_name == METRICS, (
...
def export_score_as_csv(ROOT):
    ...
    for split in ["test"]:
        ...
        save_path = os.path.join(ROOT, f"scores_{split}_unanno.tex")
